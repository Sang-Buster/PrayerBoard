...
export async function getPrayers(
  ...
  const [prayersResult, countResult] = await Promise.all([
    sql`SELECT * FROM prayers ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`,
...
export async function getStats(): Promise<Stats> {
  ...
  // Fill in missing dates with 0 counts for the last 30 days
